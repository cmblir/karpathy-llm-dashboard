...
Cytoscape.js + **d3-force** 레이아웃으로 vault 전체 링크 그래프 — Obsidian이 실제 사용하는 동일한 포스 패밀리(`forceLink` + `forceManyBody` + `forceX/Y`)를 그대로 적용합니다. 시뮬레이션은 **연속(infinite)** 모드 — 노드를 드래그하면 이웃이 따라오고, 놓으면 다시 자리를 잡습니다. 노드는 파일, 엣지는 `[[wikilinks]]`. 허브 노드는 링크 수에 따라 커지고, 미해결 wikilink 대상은 고스트 노드로, 라벨은 줌 임계값 이상에서 페이드 인됩니다.

우측 설정 드로어(톱니바퀴 아이콘)가 Obsidian 패널 구성을 그대로 따릅니다:

- **Filters** — 파일명 라이브 검색, 태그 칩, 폴더 드롭다운, *고립 노드 표시* / *존재하는 파일만* 토글.
- **Display** — *화살표* (각 링크 방향), *라벨 페이드 임계*, *노드 크기*, *링크 두께*.
- **Forces** — *중심력* (xStrength / yStrength), *반발력* (manyBodyStrength), *링크 장력* (linkStrength), *링크 거리*. 각 슬라이더가 실제 d3-force 파라미터를 직접 조작하므로 Obsidian 패널과 동일하게 시뮬레이션이 즉시 반응합니다.

노드 위에 마우스를 올리면 1-hop 이웃이 강조되고 나머지는 12 %로 디밍됩니다. 클릭하면 파일이 열립니다. 마우스 휠 + 배경 드래그로 줌·팬, 툴바에 줌인/맞춤/줌아웃과 **▶ Timelapse** 버튼 제공 — 파일 mtime 순서대로 노드를 하나씩 드러내며, vault가 실제로 자라온 속도 그대로 재생합니다. 드로어 상태와 모든 슬라이더 위치는 localStorage에 영속됩니다.
